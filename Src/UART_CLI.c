...
void vUARTCommandConsoleStart( void )
{
	 ...
	 * and to the headerfile (UART_CLI.h)  ***/
		...
		FreeRTOS_CLIRegisterCommand( &xSetShutdownTimer );
		...
		FreeRTOS_CLIRegisterCommand( &xPowerOnButtonEnable );
		FreeRTOS_CLIRegisterCommand( &xPowerOnButtonTime );
...
/*** prvPowerOnButtonEnable
 * This command enable or disables the PowerOnButton Function.
 * When the Raspberry Pi has been powered off through the "poweroff" command with the StromPi 3, this external button can be used to turn on the primary voltage source back on.
 * For further Informations, please refer to the Instructions in the PowerOnButton Manual
 *
 * ***/

static portBASE_TYPE prvPowerOnButtonEnable( int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString )
{

	uint8_t powerOnButton_enable_temp;
    ...
    powerOnButton_enable_temp = ascii2int(pcParameter1);

    powerOnButton_enable = powerOnButton_enable_temp;
    ...
    switch (powerOnButton_enable)
        {
        	case 0:
    					sprintf(( char * ) pcWriteBuffer,"The PowerOn-Button has been deactivated");
    					break;

        	case 1:
        	    		sprintf(( char * ) pcWriteBuffer,"The PowerOn-Button has been enabled");
        	    		break;
...
/*** prvPowerOnButtonTime
 * This command sets the time in seconds for the PowerOn-Button Initialization - for more Information, please refer to the Instructions in the PowerOnButton Manual
 *
 * ***/

static portBASE_TYPE prvPowerOnButtonTime( int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString )
{

	uint16_t powerOnButton_time_temp;
    ...
    powerOnButton_time_temp = ascii2int(pcParameter1);

    powerOnButton_time = powerOnButton_time_temp;

    flashConfig();

	sprintf(( char * ) pcWriteBuffer,"The Shutdown-Timer has been set to %02d seconds", powerOnButton_time);
